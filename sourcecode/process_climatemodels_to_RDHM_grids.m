...
day_yr=[365 366 365 365 365 365 366 365 365 365 366 365];
 ...
 file='input/input climate_projection_lat_lon_common_grids.txt';
          loc=importdata(file);
          grid_row=loc(:,1);
...
filename1=['RCP/',model,'/',yyyy,'/Extraction_pr.nc'];
filename2=['RCP/',model,'/',yyyy,'/Extraction_tasmax.nc'];
filename3=['RCP/',model,'/',yyyy,'/Extraction_tasmin.nc'];
...
%pcp=interp2(hrap_x,hrap_y,forcing,lon,lat)
 output_file1=['RCP/',model,'/',yyyy,'/precip/xmrg',mm,dd,yyyy,hh1,'z.asc'];
...
output_file4=['RCP/',model,'/',yyyy,'/temp_new/tair',mm,dd,yyyy,hh1,'z.asc'];
