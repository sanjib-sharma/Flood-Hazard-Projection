...
exp99 = merged[['Longitude', 'Latitude', 'exp_2099']].copy()
...
haz99_city = merged_city[['Longitude', 'Latitude', 'haz_2099']].copy()
exp99_city = merged_city[['Longitude', 'Latitude', 'exp_2099']].copy()
...
haz99_boro = merged_boro[['Longitude', 'Latitude', 'haz_2099']].copy()
exp99_boro = merged_boro[['Longitude', 'Latitude', 'exp_2099']].copy()
...
blues2 = ListedColormap(blues1(np.linspace(0.40, 1.0, 256)))
ax.pcolormesh(x, y, data[:,:].T, cmap=blues2, alpha=1)
'''
ax.pcolormesh(x2, y2, data2[:,:].T, cmap=cm.get_cmap('Reds_r'), alpha=0.1)
'''
list_circles = exp99_boro[['Latitude', 'Longitude', 'exp_2099']].dropna().values
for lat, lon, var in list_circles:
	if var<25:
		c=Circle((lon, lat), radius=5000, alpha=0.5, zorder=2, edgecolor='grey', linewidth=0.3, facecolor='darkgrey')
	elif var<50:
		c=Circle((lon, lat), radius=5000, alpha=0.5, zorder=2, edgecolor='grey', linewidth=0.3, facecolor='grey')
	elif var<75:
		c=Circle((lon, lat), radius=5000, alpha=0.5, zorder=2, edgecolor='grey', linewidth=0.3, facecolor='lightcoral')
	else:
		c=Circle((lon, lat), radius=5000, alpha=0.5, zorder=2, edgecolor='grey', linewidth=0.3, facecolor='red')
	a = ax.add_patch(c)

legend_elements = [Line2D([0], [0], marker='o', color='w', label='0-25', markerfacecolor='darkgrey', markersize=6,
markeredgecolor='grey', alpha=0.8),
...
markeredgecolor='grey', alpha=0.8)]
lgd = ax.legend(handles=legend_elements, loc='lower center', title='Exposure [%]', ncol=4, bbox_to_anchor=(0.4, -0.20, 0.2, 0.05),
fontsize=10, frameon=False, columnspacing=0)
lgd.get_title().set_fontsize('10')
fig.savefig('../maps/final_maps/exposure2099_boro_final.png', dpi=600, bbox_inches='tight')
